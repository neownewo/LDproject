...
export function createRecordSessionCookie(userId) {
...
export function clearRecordSessionCookie() {
  const secure = process.env.VERCEL ? '; Secure' : ''
  return `${COOKIE_NAME}=; Path=/; HttpOnly; SameSite=Lax; Max-Age=0${secure}`
}

export function getRecordSession(req) {
  try {
...
export function requireRecordUser(req, res) {
