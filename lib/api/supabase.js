...
export async function listRecordEntries(userId) {
  const records = await dbRequest(
    `gacha_records?select=id,pool_key,rank,pull_count,gold_count,select_box_opened,amount_twd,note,created_at,updated_at&user_id=eq.${encodeURIComponent(userId)}&order=updated_at.desc`,
  )
...
export async function upsertRecordEntry(userId, payload) {
  ...
  const rows = await dbRequest('gacha_records?on_conflict=user_id,pool_key', {
    ...
    body: JSON.stringify({
      ...
      gold_count: payload.goldCount,
      select_box_opened: payload.selectBoxOpened === true,
