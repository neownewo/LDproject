...
export async function listRecordEntries(userId) {
  const records = await dbRequest(
    `gacha_records?select=id,pool_key,rank,pull_count,gold_count,amount_twd,note,created_at,updated_at&user_id=eq.${encodeURIComponent(userId)}&order=updated_at.desc`,
  )
