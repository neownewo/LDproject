...
async function getLegacyPools() {
  ...
  return (Array.isArray(rows) ? rows : []).map(normalizeLegacyPool).filter((pool) => pool.name && pool.startDate)
}

async function getManagedPools() {
  const rows = await listPublishedGachaPools()
  return (Array.isArray(rows) ? rows : []).map(normalizeManagedPool).filter((pool) => pool.name && pool.startDate)
}
...
export async function getRecordPools() {
  ...
  return [...byKey.values()].sort((a, b) => String(b.startDate).localeCompare(String(a.startDate)))
}
