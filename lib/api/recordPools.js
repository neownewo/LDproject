...
function buildPoolCards(pool) {
  ...
  const cardNames = parseList(pool.note, /[、，,]/)

  let count = 1
  if (pool.poolType === 'daily') count = Math.max(2, images.length, cardNames.length)
  else if (pool.poolType === 'multi') count = images.length || cardNames.length || characters.length || 1

  return Array.from({ length: count }, (_, index) => ({
    cardKey: `card:${index}`,
    cardIndex: index,
    label: cardNames[index] || characters[index] || `卡片 ${index + 1}`,
